...
<div class="col-sm-offset-3 container client-service-container">
    ...
    <div class="modal-dialog" id="cModal">
        <div class="serviceModalContent modal-content" id="cModalContent">
            <div class="serviceModalBody modal-body" id="cModalBody">
                <div class="service-modal-q clearfix" id="cModalQ">
                	<div class="mqMark">Q.</div>
                	<div class="mqContent"></div>
                </div>
                <hr>
...
<script>
	...
	$('.Q1service').click(function() {
        ...
        const serviceQ1Question = `비밀번호 변경은 어디서 하나요?`;
        const serviceQ1Answer = `<strong class="answer-text">로그인 -> 마이페이지 -> 좌측 사이드바에 "비밀번호 변경" 페이지에서 자유롭게 변경 가능합니다.</strong>`;

        inputContent(serviceQ1Question, serviceQ1Answer);
    });

    $('.Q2service').click(function() {
    	
    	const serviceQ2Question = `납입증명서 발급은 어떻게 하나요?`;
        const serviceQ2Answer = `<strong class="answer-text">
        ...
        inputContent(serviceQ2Question, serviceQ2Answer);
    });
    
	$('.Q3service').click(function() {
		
		const serviceQ3Question = `진단서의 유효기간이 있나요?`;
        const serviceQ3Answer = `<strong class="answer-text">유효기간이 정해져 있지는 않지만 환자의 상태에 따라 진단과 소견이 달라질 수 있으므로 발급 후 바로 사용 하셔야 합니다.</strong>`;

        inputContent(serviceQ3Question, serviceQ3Answer);
	});
   
	$('.Q4service').click(function() {
		
		const serviceQ4Question = `회원탈퇴는 어떻게 하나요?`;
        const serviceQ4Answer = `<strong class="answer-text">로그인 -> 마이페이지 -> 좌측 사이드바에 "회원탈퇴" 페이지에서 회원 탈퇴 가능합니다.</strong>`;

        inputContent(serviceQ4Question, serviceQ4Answer);
    });
   
	$('.Q5service').click(function() {
		
		const serviceQ5Question = `진료를 하는 것이 아니고 기록 사본이 필요한데 왜 접수하고 의사를 만나야 하나요?`;
        const serviceQ5Answer = `<strong class="answer-text">
        ...
        inputContent(serviceQ5Question, serviceQ5Answer);
    });
   
	$('.Q6service').click(function() {
		
		const serviceQ6Question = `예약 현황은 어디서 확인 하나요?`;
        const serviceQ6Answer = `<strong class="answer-text">로그인 -> 마이페이지 -> 좌측 사이드바에 "예약 현황" 페이지에서 확인하실 수 있습니다.</strong>`;

        inputContent(serviceQ6Question, serviceQ6Answer);
    });
   
	$('.Q7service').click(function() {
		
		const serviceQ7Question = `군대에 들어간 아들의 의무기록 사본 발급 신청시 필요한 서류는?`;
        const serviceQ7Answer = `<strong class="answer-text">
        ...
        inputContent(serviceQ7Question, serviceQ7Answer);
    });
   
	$('.Q8service').click(function() {
		
		const serviceQ8Question = `기타 문의 사항은 어디에서 할 수 있나요?`;
        const serviceQ8Answer = `<strong class="answer-text">로그인 -> 상단 탭 "게시판" 클릭 -> 고객의 소리 페이지에서 작성해주시면 빠른 시일내에 관리자를 통해 답변 드리겠습니다.</strong>`;

        inputContent(serviceQ8Question, serviceQ8Answer);
    });
	
	function inputContent(mqContent, aContent) {
		$('.mqContent').html(mqContent);
